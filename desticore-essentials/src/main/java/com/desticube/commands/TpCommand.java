...
@Command(
		...
		aliases = {"tp2p", "teleport"},
		permissions = {"desticore.tp"})
public class TpCommand extends AbstractCommand implements Defaults {

	@Override
	public boolean onCommand(CommandSender sender, org.bukkit.command.Command cmd, String label, String[] args) {
		if (sender == Bukkit.getConsoleSender()) {sender.sendMessage(color(msg.GENERAL_PLAYER_ONLY_COMMAND)); return true;}
		DestiPlayer p = getDestiPlayer((Player) sender);
		if (!p.hasPermission("desticore.tp")) return p.sendMessage(msg.GENERAL_NO_PERMISSIONS);
		if (args.length < 1 || args == null) {return p.sendMessage(color("&cCorrect usage: /tp (player)"));}
