...
@Command(
		...
		description = "TPA",
		aliases = {},
		permissions = {"desticore.tpa"})
public class TpaCommand extends AbstractCommand implements Defaults {

	@Override
	public boolean onCommand(CommandSender sender, org.bukkit.command.Command cmd, String label, String[] args) {
		if (sender == Bukkit.getConsoleSender()) {sender.sendMessage(color(msg.GENERAL_PLAYER_ONLY_COMMAND)); return true;}
		DestiPlayer p = getDestiPlayer((Player) sender);
		if (!p.hasPermission("desticore.tpa")) return p.sendMessage(msg.GENERAL_NO_PERMISSIONS);
		if (args.length < 1) return p.sendMessage("Correct usage: /tpa (name)");
		if (p.hasSentTpRequest()) return p.sendMessage("Already have request sent");
		...
		p.setSentTpRequest(target);
		for (String s : config.TPA_RECIEVED_FORMAT) {
			destitarget.sendMessage(s.replaceAll("%player%", p.getName()));	
		}
		destitarget.removeRecievedTpRequest(destitarget.getRecievedTpRequest());
		destitarget.setRecievedTpRequest(p.getPlayer());
		TpaRequestRemove.a().startDeleteTimer(p.getPlayer(), target, 120);
		return false;
	}
